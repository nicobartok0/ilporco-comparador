...
class Valuador_Pricely(Valuador):
    ...
    def buscar_precios(self):
        for articulo in self.articulos.values():
            response = requests.get(f'https://pricely.ar/product/{articulo.cod_pricely}', cookies=self.cookies, headers=self.headers)
            soup = BeautifulSoup(response.content, 'html.parser')
            precio = soup.find('span', {'class': 'font-display text-zinc-700 text-2xl'})
            try:
                articulo.precios['PRICELY'] = precio.text
            except:
                articulo.precios['PRICELY'] = '?'